...
public class SysDeptController extends AbstractController {
    ...
    public R save(@RequestBody String data) {
        SysDeptEntity deptEntity = JsonUtils.readValue(data, SysDeptEntity.class);
        if (deptEntity.getDeptId() == null || deptEntity.getDeptId() == 0) {
...
//	}
    ...
    public R delete(@RequestBody String data) {
        ...
        //判断是否有子部门
        List<Long> deptList = sysDeptService.queryDetpIdList(idVo.getId());
        if (deptList.size() > 0) {
            return R.error("请先删除子部门");
        }

        sysDeptService.deleteById(idVo.getId());
